...
export default function AllClaims({ columns }) {
  ...
  const { data, loading, error } = useFetch(
    `http://localhost:8090/api/${path}/?limit=${PRODUCT_LIMIT}&offset=${offsetProducts}`
  );
          ...
          <div className="grid grid-cols-1 px-4 mb-16">
            <ModalAction setShowModal={setShowModal} showModal={showModal}>
              <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                <h3 className="text-3xl font-semibold">Eliminar propiedad</h3>
              </div>
              {/*body*/}
              <div className="relative p-6 flex-auto">
                <p className="my-4 text-slate-500 text-lg leading-relaxed">
                  ¿Estás seguro que deseas eliminar: <b>{currentProp.title}</b>{" "}
                  ?
                </p>
              </div>
              {/*footer*/}
              <div className="flex items-center justify-center p-6 border-t border-solid border-slate-200 rounded-b">
                ...
                <ButtonPrimary onClick={() => handleDelete(currentProp.id)}>
                  Eliminar
                </ButtonPrimary>
              </div>
            </ModalAction>
              ...
              <CardBody>
                {/* <div className="flex justify-between w-full mb-8 px-2 items-center ">
                  ...
                  <Link to={`/${path}/new`} className="link">
                    <ButtonPrimary>Agregar nueva</ButtonPrimary>
                  </Link>
                </div> */}
